...
class TransferController extends Controller
{
    ...
    {
        $this->paymentMethod = PaymentMethod::where('code', 'cuan')->first();
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $transactionType = TransactionType::whereIn('code', ['transfer', 'receive'])
                ->orderBy('code', 'asc')
                ->get();

            $senderTransactionType = $transactionType->first();
            $receiveTransactionType = $transactionType->last();

            // Create transaction for sender
            $this->createTransaction([
                'user_id' => $sender->id,
                'transaction_type_id' => $senderTransactionType->id,
