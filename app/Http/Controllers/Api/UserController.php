...
class UserController extends Controller
{
    ...
    public function getUserByUsername(Request $request, $username)
    {
        ...
        $users->map(function ($item) {
            $item->profile_picture = $item->profile_picture ?
                url('storage/profile/' . $item->profile_picture) : "";
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $data = $request->only('name', 'username', 'ktp', 'profile_picture', 'email', 'password');
            ...
            if ($request->profile_picture) {
                $profilePicture = uploadImgBase64($request->profile_picture, $request->username, 'profile');
            ...
            if ($request->ktp) {
                $ktp = uploadImgBase64($request->profile_picture, $request->username, 'ktp');
                $data['ktp'] = $ktp;
                $data['verified'] = true;
                if ($user->ktp) {
                    Storage::delete('public/storage/ktp/' . $user->ktp);
                }
            }
